                                ...
                                ?>
                                    <div class="chapter-item bg-white border border-slate-200 rounded-md p-3 shadow-sm">
                                        <div class="flex items-center gap-2 mb-2">
                                            ...
                                            <span class="text-sm font-bold text-slate-700"><?php echo $chap['chapter_title']; ?></span>
                                        </div>
                                        <div class="sub-chapter-list space-y-1.5">
                                            <?php while ($sub = mysqli_fetch_assoc($sub_res)) { ?>
                                                <div class="flex items-center gap-2 pl-3 border-l-2 border-slate-200 text-xs text-slate-500">
...
<script>
    $(document).ready(function() {
        ...
        $('#addNewModule').off('click').on('click', function() {
            ...
            <div class="module-card bg-white border border-slate-200 rounded-md shadow-sm flex flex-col transition-all hover:border-indigo-300 animate-slideUp">
                <div class="p-5">
                    ...
                    <input type="text" placeholder="Module Title..." class="mod-title w-full bg-transparent font-bold text-slate-700 text-lg outline-none border-b-2 border-transparent focus:border-indigo-200 pb-1 mb-2">
                    <div class="flex items-center justify-between mt-4">
        ...
        $(document).off('click', '.toggle-module-btn').on('click', '.toggle-module-btn', function(e) {
            ...
            // Close all other open modules first
            $('.module-body').not(body).stop(true, true).slideUp(200);
            $('.toggle-module-btn i').not(icon).removeClass('rotate-180');
        ...
        $(document).off('click', '.add-chapter-btn').on('click', '.add-chapter-btn', function(e) {
            ...
            <div class="chapter-item bg-white border border-slate-200 rounded-md p-3 shadow-sm mb-3">
                <div class="flex items-center justify-between mb-2">
                    <input type="text" placeholder="Chapter title..." class="ch-title text-sm font-bold text-slate-700 bg-transparent outline-none w-full border-b border-transparent focus:border-emerald-200">
        ...
        $(document).off('click', '.add-sub-btn').on('click', '.add-sub-btn', function(e) {
            ...
            <div class="sub-chapter-item flex items-center justify-between gap-2 pl-3 border-l-2 border-slate-200">
                <input type="text" placeholder="Topic name..." class="sub-title text-xs text-slate-500 bg-transparent outline-none w-full py-1">
        ...
        // Hide if click outside of module cards
        $(document).on('click', function(e) {
            if (!$(e.target).closest('.module-card').length) {
                $('.module-body').stop(true, true).slideUp(200);
        ...
        $(document).off('click.saveModule').on('click.saveModule', '.save-btn', function(e) {
            ...
            // Validation: Don't even send to PHP if basics are empty
            if (!data.module_code || !data.module_title) {
                showToast("Validation Error", "error", "Please enter both Module Code and Title.");
                return;
            }

            // Gather Chapters and Sub-topics
            ...
            $.ajax({
                ...
                error: function() {
                    showToast("System Error", "error", "Could not connect to the server.");
                    btn.removeClass('is-saving').html('<i class="fas fa-save text-sm"></i>');
                }
            });
        });
